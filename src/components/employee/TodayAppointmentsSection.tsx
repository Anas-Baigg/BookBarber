...
// Both isoTime and now are UTC — express them in the shop's timezone so the
// difference is calculated against the shop's local clock, not the server's.
...
export default function TodayAppointmentsSection({ todayBookings, nextAppointment, timezone }: Props) {
  ...
  async function transition(bookingId: string, newStatus: BookingStatus) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error('[status transition]', data.error ?? res.status);
      }
    } catch (err) {
      console.error('[status transition]', err);
    }
  ...
  // "next appointment is today" — compare dates in the shop's timezone, not the
  // client's OS timezone (which toDateString() would use).
  ...
  // Check-in window: within 30 minutes of start or already past (shop time)
  ...
  return (
    <div>
      ...
      {nextAppointment ? (
        <div className="mb-6 p-4 bg-dark-100 border border-gold/30 rounded-2xl flex items-center justify-between gap-4">
          ...
          {/* Check In button: appears when within 30 min in the shop's timezone */}
          {(toZonedTime(parseISO(nextAppointment.start_time), timezone).getTime() - nowInZone.getTime()) <= THIRTY_MIN_MS_VAL &&
           getStatus(nextAppointment) === 'confirmed' && (
            <button
              onClick={() => transition(nextAppointment.id, 'checked_in')}
              disabled={transitioning === nextAppointment.id}
              className="flex-shrink-0 flex items-center gap-1.5 px-4 py-2 bg-gradient-gold text-dark text-sm font-semibold rounded-lg hover:opacity-90 transition-opacity disabled:opacity-50"
            >
      ...
      {todayBookings.length === 0 ? (
        ...
        <div ref={timelineRef} className="space-y-2">
          {todayBookings.map((b, idx) => {
            ...
            // Express slot start in the shop's timezone for all comparisons
            const startZoned  = toZonedTime(parseISO(b.start_time), timezone);
            // Fix 3: "done" dimming uses shop-timezone reference for current time
            const isPast      = startZoned < nowInZone;
            // pending_reschedule: slot is freed, awaiting customer — treat as done visually (Fix 11)
            const isDone      = ['completed', 'no_show', 'cancelled', 'pending_reschedule'].includes(status);
            // Fix 4: 30-minute check-in window calculated in shop timezone
              ...
              <div key={b.id}>
                ...
                >
                  ...
                  <div className="flex flex-col items-end gap-1 flex-shrink-0 ml-3">
                    ...
                    {/* Action buttons — today only, contextual on status and shop time */}
                    {!isDone && (
                      <div className="flex gap-1.5 mt-1">
                        {status === 'confirmed' && (withinWindow || isPast) && (
                          <button
                            ...
                            disabled={busy}
                            className="flex items-center gap-1 px-2.5 py-1 text-xs font-medium bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 rounded-lg hover:bg-cyan-500/20 transition-colors disabled:opacity-50"
                          >
                        ...
                        {status === 'checked_in' && (
                          <>
                            <button
                              ...
                              disabled={busy}
                              className="flex items-center gap-1 px-2.5 py-1 text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-lg hover:bg-emerald-500/20 transition-colors disabled:opacity-50"
                            >
                              <CheckCircle className="w-3 h-3" />
                              {busy ? '…' : 'Complete'}
                            </button>
                            <button
                              ...
                              disabled={busy}
                              className="flex items-center gap-1 px-2.5 py-1 text-xs font-medium bg-gray-500/10 text-gray-400 border border-gray-500/20 rounded-lg hover:bg-gray-500/20 transition-colors disabled:opacity-50"
                            >
                        ...
                        {status === 'confirmed' && isPast && !withinWindow && (
                          <button
                            onClick={() => setConfirmNoShow(b.id)}
                            disabled={busy}
                            className="flex items-center gap-1 px-2.5 py-1 text-xs font-medium bg-gray-500/10 text-gray-400 border border-gray-500/20 rounded-lg hover:bg-gray-500/20 transition-colors disabled:opacity-50"
